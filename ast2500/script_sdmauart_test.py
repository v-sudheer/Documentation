...
def target_log_write(log_string):
    with open('ASTlogfile','a') as log_file:
...
debug_uart_port.write("uname -a\n");

line = debug_uart_port.readline();
if line:
	print(line)
...
child.send("pwd\r")
child.send("cd /tftpboot/suvelise/Apr23_2019_uart_test\r")
child.send("rm -f test1\r")
child.send("rm -f recv_file1\r")
child.send("rm -f ASTlogfile\r")
...
debug_uart_port.write("stty -F  /dev/ttyDMA2 115200 -onlcr -echo\n");
child.send("stty -F /dev/ttyUSB1 115200 -onlcr -echo\r")
...
#for iter_val in range(5):
	...
	debug_uart_port.write("cat /dev/ttyDMA2 > recv_file1\n");
	debug_uart_port.timeout = 1 
	while 1:
		serial_line = debug_uart_port.readline()
		#print serial_line
	...
	child.send("cat test1 > /dev/ttyUSB1\r")
	serial_line = child.readline()
	host_log_write(serial_line)
	while 1:
		serial_line = debug_uart_port.readline()
	...
	child.send("cat /dev/ttyUSB1 > recv_file1\r")
	serial_line = child.readline()
	host_log_write(serial_line)

	debug_uart_port.write("cat recv_file1 > /dev/ttyDMA2\n");
        debug_uart_port.timeout=calc_read_timeout(file_size)
	while 1:
		serial_line = debug_uart_port.readline()
		#print serial_line
